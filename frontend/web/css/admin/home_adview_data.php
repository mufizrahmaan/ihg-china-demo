...
if($show == 'today')
{
	...
	$stmt = $db->query($sql);
	$count = $stmt->rowCount(); 
    $view_data[0] = array('Time','Male','Female','Total', 'Footfall');
	$i = 1;

	foreach( $period as $hour) {
		$key = $hour->format( 'H');
		$view_data[$i] = array($key,(int)0,(int)0,(int)0,(int)0);
		$i++;
	}
	...
	$sql1 = "select * from ad_footfall where collected_time > $todaytime and collected_time is not null order by collected_time";
	$stmt1 = $db->query($sql1);
	while($row1 = $stmt1->fetch())
	{
		$collected_time_milliseconds = $row1['collected_time'];
		$collected_time_seconds = $collected_time_milliseconds / 1000;
		if($collected_time_seconds)
		{
			$display_date = date('H', $collected_time_seconds);
...
else if(($show == 'monthly') || ($show == 'last3months'))
{
	...
	$stmt = $db->query($sql);
	$count = $stmt->rowCount(); 
	$view_data[0] = array('Month','Male','Female','Total', 'Footfall');
	$i = 1;
	...
	$sql1 = "select * from ad_footfall where collected_time is not null order by collected_time";
	$stmt1 = $db->query($sql1); 
	while($row1 = $stmt1->fetch())
	{
		$collected_time_milliseconds = $row1['collected_time'];
	...
	$stmt = $db->query($sql);
	$count = $stmt->rowCount(); 
	$view_data[0] = array('Date','Male','Female','Total','Footfall');
	$i = 1;
