...
if ($showForm == true) {
	if (isset ( $_POST ['Submit'] ) && $_POST ['Submit'] == 'Submit') {
		...
		if (isset ( $_POST ['id'] ) && ! empty ( $_POST ['id'] ) && $error == '') {
			...
			$post_account_balance = $pre_account_balance + $credit_amount;
			$logged_in_admin_id = $_SESSION['user_id'];

			$sql = "INSERT INTO transaction_history ( amount, type, company_id, date, user_id ,pre_account_balance,post_account_balance,notes) VALUES ('$credit_amount', 'CREDIT', '$id', '$created_date','$logged_in_admin_id','$pre_account_balance','$post_account_balance','$notes')"; 
			$result = $db->exec ( $sql );
			$transaction_history_id = $db->lastInsertId();

			$sql = "UPDATE company_registration SET 
					account_balance = '".$post_account_balance."'
					WHERE  id = '" . $_POST ['id'] . "'";
			$result = $db->exec ( $sql );
...
<div class="main-content">
	...
	<div class="page-content">
		<div class="page-header">
			<h1>
				Credit Note <small> <i class="icon-double-angle-right"></i>
									 <?php
										if (isset ( $row ['id'] )) {
 ...
 <?php
    ...
    <?php } ?>
						...
						<div class="control-group">
							<label class="control-label" for="inputemail_id">Credit Amount:<font
						...
						<div class="control-group">
							<div class="form-actions">
								...
								<i class="icon-save"></i> Apply Credit
								</button>
