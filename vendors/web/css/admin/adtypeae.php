...
if ($show_form == true)
{
	...
	{
		if (trim($_POST['ad_type']) != '')
            $ad_type = trim($_POST['ad_type']);
        else
            $error .= '<strong><i class="icon-remove"></i></strong>&nbsp;AD Type is a required field.<br/>';
        ...
        if (isset($_POST['id']) && !empty($_POST['id']) && $error == '') {
            ...
            header('Location: adtype.php');
            exit();
        } else if ($error == '') {
            $sql = "INSERT INTO ad_type ( 
									ad_type,
									active,
									created_date,
									updated_date
...
<!-- main content starts from here -->
 ...
 <?php
    ...
    <label class="control-label" for="inputEmail">AD Type:<font color="#FF0000">*</font></label>
    <div class="controls">
    <input type="text" name="ad_type" id="ad_type" <?php echo $extrastring; ?> required value="<?php echo isset($ad_type) ? $ad_type : ''; ?>" />&nbsp;&nbsp;<div class="help-block inline"></div>
    </div>
    </div>
   ...
   <div class="form-actions">
	 ...
	 <a href="adtype.php" class="btn btn-sm"><i class="icon-remove"></i> Cancel</a>
     <input type="hidden" name="id" value="<?php echo isset($_GET['id']) ? (int) $_GET['id'] : ''; ?>"/>
	 <br/><br/><a href="adtype.php"><i class="icon-reply"></i> Back to AD Types</a>
   </div>
   </div>
