...
if ( isset($user_id) )
{
	...
	{
		if($page_name == "Ads" || $page_name == "Devices"){
		$sort_by = $column[1]['db_field'];
		$sort_order = trim("asc");		
		} else {		
	...
	if($page_name == "Ads"){
	    $query_where .= " group by ad.id";
	}
	if($page_name == "Devices"){
	    $query_where .= " and active = 1";
	}	
	  ...
	  <!-- <caption>Data</caption>-->
							...
							<?php
								...
								foreach ($column as $array_column) 
								{
									if($array_column['label'] == "Action")
									{
										...
										$colspan = $array_column['colspan'] + 2;
					...
					if ($total > 0)
					{
						do
						{
							...
							foreach ($column as $array_column) 
							{
									//print_r($array_column); echo count($array_column); exit;
									...
									if(isset($array_column['db_field']) || isset($array_column['action_field']))
									{
										...
										if(isset($array_column['href']))
										{
											...
											if(isset($array_column['action_field']))
											{
												...
												else if($array_column['action_field'] == 'Users')
												{
														$value .= "<i class=\"icon-user blue bigger-130\"></i>";
												}
											} 
											else
											{
												if(isset($array_column['db_field_type']) && $array_column['db_field_type'] == "image")
												{
													$value .= "<a ".$target." rel=\"tooltip\" data-placement=\"right\" href=\"".$array_column['href'].$row['id']."\">"."<img width=\"80px\" src=\"http://shelf.mintm.com/manage/".$row[$array_column['db_field']]."\" alt=\"".$row[$array_column['db_field']]."\" border=\"0\" />";
												}
												else
												{
										...
										// date time
										else if($array_column['db_field_type'] == "datetime" || $array_column['db_field'] == "date_added" || $array_column['db_field'] == "date_modified")
										{
										...
										{
											$value .= "<img width=\"80px\" src=\"http://shelf.mintm.com/manage/".$row[$array_column['db_field']]."\" alt=\"".$row[$array_column['db_field']]."\" border=\"0\" />";
										}
									...
									if($page_name == "Ads"){
										...
										if($array_column['db_field'] == 'deleted'){
											    if($row['deleted'] == 1 && $row['active'] == 0){
												    $value = "Approval Rejected";
												} else if($row['deleted'] == 0 && $row['active'] == 1){
												    $value = "Approval Done";
												} else if($row['deleted'] == 0 && $row['active'] == 0) {
                                                    $value = "Approval Time Completed";
												}												
										}				
									
									}
									if($page_name == "Publishers"){
									    if($array_column['db_field'] == 'View Establishments'){
											$sql_device = "SELECT count(*) as count FROM company_establishment WHERE company_id = '".$row['id']."' AND display = 1";
											$stmt_device = $db->query($sql_device);
											$store = $stmt_device->fetch();				
									        $value .= '<a>('.$store['count'].')</a>';
										}
									}						
                                    if($page_name == "Devices"){
											$sql_device = "SELECT * FROM view_data_filtered WHERE device_id = ".$row['id']." ORDER BY created_date DESC LIMIT 0,1";
											$stmt_device = $db->query($sql_device);
											$row_device = $stmt_device->fetch();

											$batteryState = $row_device['batteryState'];
											$updatedDate = $row_device['created_date'];
											if($array_column['label'] == 'Battery State'){
												if($batteryState == '1.0' || $batteryState == '1')
												{
													$batteryState = 'Charging';
													$batteryStateText = '<button class="btn btn-xs btn-success">
													<i class="icon-signal bigger-110"></i>&emsp;'.$batteryState.'</button>';
													$value = $batteryStateText;
												}
												else
												{
													$batteryState = 'Discharging';
													$batteryStateText = '<button class="btn btn-xs btn-danger">
													<i class="icon-ban-circle bigger-110"></i>'.$batteryState.'</button>';
													$value = $batteryStateText;
												}
											}
                                            if($array_column['label'] == 'Last updated'){
                                                 $value = $updatedDate; 
											}										
