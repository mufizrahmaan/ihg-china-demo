...
$sql = "SELECT * FROM advertiser_role ORDER BY role";
$stmt = $db->query($sql);
...
$is_advertising_agency = array(0=>'No',1=>'Yes');

$all_advertising_agency = array('0' => '--None--');
$sql = "SELECT * FROM company_registration WHERE advertising_agency = 1 ORDER BY name asc";
$stmt = $db->query($sql);
while ($row = $stmt->fetch()) {
     $all_advertising_agency[$row['id']] = $row['name'];
}
...
if ($showForm == true) {
    if (isset($_POST['Submit']) && $_POST['Submit'] == 'Submit') {
        if (trim($_POST['name']) != '' && strlen($_POST['name']) > 3)
            ...
            $error .= '<strong><i class="icon-remove"></i></strong>&nbsp;Advertiser Name is a required field.<br/>';
		...
		$advertising_agency = trim($_POST['advertising_agency']);
        ...
        if (isset($_POST['id']) && !empty($_POST['id']) && $error == '') {
			$id = $_POST['id'];
				...
				else
				{
					...
					//$advertiser_id = strtoupper(substr(clean($name), 0, 3)."-".randomNumber(10));
					$advertiser_id = "adv-".big_rand(10);
					...
					$to      = $email_id;
					$subject = 'New Advertiser Added in Shelf - Login Details';
					$message = 'New Advertiser Added in Shelf <br/><br/>'.'Advertiser Name : '.$name.'<br/><br/>Username : '.$email_id.'<br/><br/>Password : '.$password;
    ...
    if (isset($_GET['id'])) {
		...
		$advertising_agency = $row_result['advertising_agency'];
...
<!-- main content starts from here -->
				...
				<div class="main-content">
					<div class="breadcrumbs" id="breadcrumbs">
						...
						<ul class="breadcrumb">
							...
							<li class="active"><?php echo $action; ?> Advertiser</li>
						</ul><!-- .breadcrumb -->
 ...
 <?php
	...
	?>
	 ...
	 <div class="control-group">
    <label class="control-label" for="inputPassword">Is Advertising Agency:<font color="#FF0000">*</font></label>
    <div class="controls">
    <select name="advertising_agency" required>
															<?php
															foreach ($is_advertising_agency as $index => $value) {
																$extra = '';
																if (isset($advertising_agency)) {
																	if ($advertising_agency == $index)
	 ...
	 <div class="control-group">
    <label class="control-label" for="inputPassword">Advertising Agency:</label>
    <div class="controls">
    <select name="parent_id" required>
															<?php
															foreach ($all_advertising_agency as $index => $value) {
																$extra = '';
																if (isset($parent_id)) {
																	if ($parent_id == $index)
																		$extra = 'selected = "selected"';
   ...
   <div class="form-actions">
	 ...
	 <a href="advertiser.php" class="btn btn-sm"><i class="icon-remove"></i> Cancel</a>
     <input type="hidden" name="id" value="<?php echo isset($_REQUEST['id']) ? (int) $_REQUEST['id'] : ''; ?>"/>
	 <br/><br/><a href="advertiser.php"><i class="icon-reply"></i> Back to Advertisers</a>
   </div>
   </div>
