...
$array_all_access = array('NO_ACCESS'=>"No Access", 'READONLY_ACCESS'=>"ReOnly Access", 'RESTRICTED_ACCESS'=>"Restricted Access",'FULL_ACCESS'=>"Full Access");

$page_name = "Category";
$page_title_bar = "Category";
$showForm = true;
...
if ($show_form == true)
{
		...
		$array_parent_category = array('0' => '--- Select Parent Category ---');
		$sql = "SELECT * FROM category where parent_id = 0 and id != '$_REQUEST[id]' order BY category";
		$stmt = $db->query($sql);
		while($row = $stmt->fetch())
		{
			$array_parent_category[$row['id']] = $row['category'];
		}
	...
	if (isset($_POST['Submit']) && $_POST['Submit'] == 'Submit')
	{
		if (trim($_POST['category']) != '')
            ...
            $error .= '<strong><i class="icon-remove"></i></strong>&nbsp;Category is a required field.<br/>';
...
<!-- main content starts from here -->
 ...
 <?php
    ...
    <label class="control-label" for="inputEmail">Category:<font color="#FF0000">*</font></label>
    <div class="controls">
    <input type="text" name="category" id="category" <?php echo $extrastring; ?> required value="<?php echo isset($category) ? $category : ''; ?>" />&nbsp;&nbsp;<div class="help-block inline"></div>
    </div>
    </div>
    ...
    <div class="controls">
    <select <?php echo $extrastring; ?> name="parent_id" id="parent_id">
										<?php
											foreach($array_parent_category as $index => $value)
											{
												$extra = '';
												if(isset($parent_id))
												{
   ...
   <div class="form-actions">
	 ...
	 <a href="category.php" class="btn btn-sm"><i class="icon-remove"></i> Cancel</a>
     <input type="hidden" name="id" value="<?php echo isset($_GET['id']) ? (int) $_GET['id'] : ''; ?>"/>
	 <br/><br/><a href="category.php"><i class="icon-reply"></i> Back to Categories</a>
   </div>
   </div>
